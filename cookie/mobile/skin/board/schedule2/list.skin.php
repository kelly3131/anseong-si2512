...
<table class="year-schedule" width="<?php echo $width; ?>" border=0 cellpadding="0" cellspacing="0">
    <tr>
        <td width="20%">&nbsp;</td>
            ...
            <table border="0" cellspacing="5" cellpadding="0">
                <tr>
                    <td>
						...
						?>"><img src="<?php echo $board_skin_url; ?>/img/y_prev.svg" border="0" alt="<?php echo $year_pre; ?>년"></a>
                    </td>
                    <td>
                    	<a class="icon-btn01 pd-inline10" href="<?php 
							echo $_SERVER['PHP_SELF']."?bo_table={$bo_table}&";
							if ($month === 1) { 
								$year_pre  = $year-1; 
								$month_pre = 12; 
							...
							echo "year={$year_pre}&month={$month_pre}&sc_no={$sc_no}";
						?>"><img src="<?php echo $board_skin_url; ?>/img/m_prev.svg" border="0" alt="<?php echo $month_pre; ?>월"></a>
                    </td>
                    <td class="month-year">
                    	<a href="<?php 
							echo $_SERVER['PHP_SELF']."?bo_table=".$bo_table; 
                    ...
                    <td>
                    	<a class="icon-btn01 pd-inline10" href="<?php 
							...
							echo "year={$year_pre}&month={$month_pre}&sc_no={$sc_no}";
						?>"><img src="<?php echo $board_skin_url; ?>/img/m_next.svg" border="0" alt="<?php echo $month_pre; ?>월"></a>
                    </td>
                    <td>
                    	<a class="icon-btn01" href="<?php 
							echo $_SERVER['PHP_SELF']."?bo_table={$bo_table}&";
							...
							echo "year={$year_pre}&month={$month_pre}&sc_no={$sc_no}";
						?>"><img src="<?php echo $board_skin_url; ?>/img/y_next.svg" border="0" alt="<?php echo $year_pre; ?>년"></a>
                    </td>
                </tr>
            </table>			
        </td>
...
<div id="bo_list">
<table class="table-schedule" width="<?php echo $width?>" bgcolor="#cfcfcf" border="0" cellspacing="1" cellpadding="5">
    <thead>
        <tr align="center">     
    ...
    while ($row = sql_fetch_array($result)) {  // 제목글 뽑아서 링크 문자열 만들기..
		...
		for ($iz = 1; $iz <= $lastcount; $iz++) { // 42번을 칠하게 된다.
			...
			if ($b_year==$year && $b_mon==$month && $b_day==$cday) $bgcolor = "#FEF4DE";      //  "#FEF4DE"; // 오늘날짜
			if (($iz%7) == 1) echo ("        <tr>\n"); // 주당 7개씩 한쎌씩을 쌓는다.
			...
			$daycontcolor = "" ; 
			$daycolor = ""; 
			if ($iz%7 == 1) $daycolor = "#F15D16"; // 일요일
			if ($iz%7 == 0) $daycolor = "#0073C2"; // 토요일
			...
			for($i=0 ; $i < sizeof($dayfile) ; $i++) {  // 파일 첫 행부터 끝행까지 루프
				...
				if($memday == $year.$arrDay[0]) {
					...
					if(substr($arrDay[2],0,3)=="red") $daycolor = "#F15D16"; // 공휴일은 날짜를 빨간색으로 표시
					if(substr($arrDay[2],0,3)=="#F15D16") $daycolor = "#F15D16"; // 공휴일은 날짜를 빨간색으로 표시
				}
			}
		...
		else { echo ("            <td width=\"{$col_width}\" height=\"{$col_height}\" bgcolor=\"#f9f9f9\" valign=\"top\">&nbsp;</td>\n"); }
		if (($iz%7) == 0) echo ("        </tr>\n");
